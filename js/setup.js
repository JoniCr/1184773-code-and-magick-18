...
(function () {
  ...
  var getRandomName = function (names, lastnames) {
    return window.getRandomElmFromArr(names) + ' ' + window.getRandomElmFromArr(lastnames);
  ...
  var getWizards = function (wizardsAmount) {
    for (var i = 0; i < wizardsAmount; i++) {
      wizards[i] = getRandomWizard(getRandomName(NAMES, LAST_NAMES), window.getRandomElmFromArr(COAT_COLORS), window.getRandomElmFromArr(EYES_COLORS));
    }
    return wizards;
  };
