...
var BAR_TEXT_Y = CLOUD_Y + CLOUD_HEIGHT - GAP;
var BAR_Y = BAR_HEIGHT + BAR_GAP;
...
window.renderStatistics = function(ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
   ...
   ctx.fillRect(BAR_X + (BAR_GAP) * i, BAR_Y, BAR_WIDTH, (BAR_HEIGHT * times[i]) / maxTime);
   ctx.fillText(Math.floor(times), BAR_TEXT_X + (BAR_GAP) * i, BAR_TEXT_Y - BAR_HEIGHT - BAR_WIDTH + GAP);
  }
